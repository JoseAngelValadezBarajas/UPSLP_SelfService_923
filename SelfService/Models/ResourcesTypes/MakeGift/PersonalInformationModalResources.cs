// --------------------------------------------------------------------
// <copyright file="PersonalInformationResources.cs" company="Ellucian">
//     Copyright 2020 - 2023 Ellucian Company L.P. and its affiliates.
// </copyright>
// --------------------------------------------------------------------

using SelfService.Models.ResourcesTypes.Admissions;

namespace SelfService.Models.ResourcesTypes.MakeGift
{
    public class PersonalInformationModalResources
    {
        public AddressSearchModal AddressSearchModal { get; set; }
        public string BtnAddNew { get; set; }
        public string BtnBack { get; set; }
        public string BtnDelete { get; set; }
        public string BtnFinish { get; set; }
        public string BtnNext { get; set; }
        public string BtnSearch { get; set; }
        public string FormatDateOutOfRange { get; set; }
        public string FormatGovernmentIdExample { get; set; }
        public string FormatGovernmentIdInvalid { get; set; }
        public string FormatTotalDonation { get; set; }
        public string LblAddressInformationStepTitle { get; set; }
        public string LblAddressLine1 { get; set; }
        public string LblAddressLine1Required { get; set; }
        public string LblAddressLine2 { get; set; }
        public string LblAddressLine3 { get; set; }
        public string LblAddressLine4 { get; set; }
        public string LblAddressType { get; set; }
        public string LblAddressTypeRequired { get; set; }
        public string LblBirthDate { get; set; }
        public string LblCity { get; set; }
        public string LblCityRequired { get; set; }
        public string LblCompleteProfileInstructions { get; set; }
        public string LblCompleteProfileTitle { get; set; }
        public string LblContactInformationStepTitle { get; set; }
        public string LblCountry { get; set; }
        public string LblCountryRequired { get; set; }
        public string LblCounty { get; set; }
        public string LblCountyRequired { get; set; }
        public string LblDescription { get; set; }
        public string LblDisplayName { get; set; }
        public string LblEmail { get; set; }
        public string LblEmailInvalid { get; set; }
        public string LblEmailRequired { get; set; }
        public string LblErrorPhoneDuplicated { get; set; }
        public string LblFirstName { get; set; }
        public string LblFirstNameRequired { get; set; }
        public string LblFormerLastName { get; set; }
        public string LblGovernmentId { get; set; }
        public string LblHouseNumber { get; set; }
        public string LblLastName { get; set; }
        public string LblLastNamePrefix { get; set; }
        public string LblLastNameRequired { get; set; }
        public string LblMiddleName { get; set; }
        public string LblNickname { get; set; }
        public string LblPersonalInformationStepTitle { get; set; }
        public string LblReCaptchaRequired { get; set; }
        public string LblPhoneCountry { get; set; }
        public string LblPhoneCountryRequired { get; set; }
        public string LblPhoneInstuctions { get; set; }
        public string LblPhoneNumber { get; set; }
        public string LblPhoneNumberInvalid { get; set; }
        public string LblPhoneNumberRequired { get; set; }
        public string LblPhoneType { get; set; }
        public string LblPhoneTypeRequired { get; set; }
        public string LblPrefix { get; set; }
        public string LblPrimary { get; set; }
        public string LblState { get; set; }
        public string LblSuffix { get; set; }
        public string LblZipCode { get; set; }
    }
}